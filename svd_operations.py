...
def svd_norm_node(node,D_cap=None,norm_dir=None):
    if node.legs == "left":
        ...
        if D_cap is not None:
            B=B[0:D_cap,:]
            B=B.reshape(np.array((D_cap,shape[0])))
        else:
            B=B.reshape(np.array((shape[1],shape[0])))
        return B.transpose()

    elif node.legs == "right":
        shape = np.shape(node.tensor)
        M = node.tensor.reshape(np.array((shape[0]*shape[1],1)))
        A,S,Vh = np.linalg.svd(M,full_matrices=False)
        if D_cap is not None:
            A = A[:,0:D_Cap]
            A = A.reshape(np.array((shape[0],D_cap)))
        else:
            A = A.reshape(np.array((shape[0],shape[1])))
        return A
        ...
        if norm_dir == "left":
            ...
            shape = np.shape(node.tensor)
            reshaped_node = np.reshape(np.einsum('ijk->jki',node.tensor),np.array((shape[1],shape[0]*shape[2])))
